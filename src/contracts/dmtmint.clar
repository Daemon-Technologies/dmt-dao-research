...
(define-public (mine-tokens (amount-ustx uint) (memo (optional (string-ascii 34))))
    (begin
        (let
            ...
            (try! (mine-tokens-at-block next-block-height (get-or-create-miner-id tx-sender) amount-ustx))
            (try! (mine-tokens-at-block (+ next-block-height u1) (get-or-create-miner-id tx-sender) amount-ustx))
            (try! (mine-tokens-at-block (+ next-block-height u2) (get-or-create-miner-id tx-sender) amount-ustx))
            (try! (mine-tokens-at-block (+ next-block-height u3) (get-or-create-miner-id tx-sender) amount-ustx))
            (try! (mine-tokens-at-block (+ next-block-height u4) (get-or-create-miner-id tx-sender) amount-ustx))
        )
        (ok true)
    )
)
